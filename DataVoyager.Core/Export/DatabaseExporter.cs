        ...
        , CancellationToken cancellationToken = default)
    {
            ...
            .Where(o => o.IsSystemObject == false))
        {
            ...
            foreach (Table o in db.Tables
                ...
                .Where(o => o.IsSystemObject == false))
            {
                _logger.LogInformation($"Export table data - {o.Name}");

                if (ignoreObjects.Any(_ => _.Equals(o.Name, StringComparison.InvariantCultureIgnoreCase)))
                {
                    _logger.LogInformation($"Table {o.Name} is ignored.");
                    continue;
                }

                string outputPathFile = Path.Combine(output.BuildPath, o.Name);
                ...
                var script = scripter.EnumScript(new SqlSmoObject[] { o });

                var ts = "";
                foreach (var s in script)
                    ts += s + Environment.NewLine;
                outputWriter.Write(ts + ";" + Environment.NewLine);
